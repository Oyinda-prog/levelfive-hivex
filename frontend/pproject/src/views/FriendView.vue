...
<template>
  <!-- 1. Added a class here to explicitly control the navbar layer -->
  ...
  <div class="container-fluid">            
    <div class="">       
       <div class="row">              
          ...
          <!-- 2. Replaced the 6 empty spacer divs with a clean padding wrapper class: 'main-content' -->
          <div class="col-12 col-lg-9 offset-md-3 main-content">             
             <div v-if="load===true" class="mx-auto col-12 col-md-9 ">       
                <div class="text-center mt-5"> 
                   <img src="../assets/loading.png" alt="" class="d-block mx-auto " width="100px">       
                </div> 
       ...
       <div class="col-12 col-lg-9 offset-md-3 d-flex justify-content-center align-items-center" style="min-height: calc(100vh - 100px); background: linear-gradient(to bottom, rgba(245, 249, 255, 0.8), rgba(214, 228, 245, 0.8));" v-if="this.msg!==''">   
          <h3 class="text-center text-primary fs-4 fw-bold">{{ msg }}</h3>      
          <button class=" ms-2 btn btn-outline-primary" @click="this.$router.push('/')">Follow friends</button> 
       </div>   
    </div>  
...
export default {
    ...
    mounted(){       
       ...
       this.studentid=JSON.parse(localStorage.getItem('studentid'))   
       axios.get(`https://onrender.com{this.studentid}`)   
       .then((res) => {     
          console.log(res.data.student);      
          this.profilepicture=res.data.student.profilepicture     
          this.fullname=res.data.student.fullname       
       ...
       axios.get(`https://onrender.com{this.studentid}`).then((res)=>{           
          this.load=false           
          this.allfriends=res.data.friends           
          console.log(this.allfriends);                          
       }).catch((err)=>{              
...
/* Fix 1: Force Navbar to stay on top layer */
.custom-navbar {
  position: sticky;
  top: 0;
...
/* Fix 2: Tweak the sidebar's layer priority and layout top offset */
.childone {   
  position: fixed;   
  top: 75px; /* Adjust this number up or down to align below your exact navbar */   
  left: 0;   
  overflow-y: auto;   
...
/* Fix 3: Added clean spacing to push loader/friends safely under navbar */
.main-content {
  padding-top: 25px;
}
</style>
